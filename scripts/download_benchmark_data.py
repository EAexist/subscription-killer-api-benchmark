...
def download_latest_release_data(repo_owner, repo_name, output_dir="results/reports"):
    ...
    try:
        ...
        if response.status_code == 404:
            ...
            print(f"💡 First run the main benchmark workflow to create a release")
            print(f"📥 Or create a manual release with your local data:")
            print(f"   gh release create benchmark-initial --title 'Initial Benchmark Data' results/ai-benchmark/benchmark-test/")
            return False
        ...
        # Files to download
        files_to_download = ['ai-metrics.csv', 'supplementary-metrics.csv']
...
def main():
    ...
    parser.add_argument('--repo-name', default='subscription-killer-api-benchmark', help='GitHub repository name (default: subscription-killer-api-benchmark)')

#!/usr/bin/env python3
"""
Trace-based Benchmark Comparison Script
Compares performance across different commits using Zipkin trace data.
"""
...
def get_output_filename(benchmark_dirs: List[Tuple[str, str]]) -> str:
    ...
    if not benchmark_dirs:
        return "ai-benchmark_comparison.md"

    first_commit = benchmark_dirs[0][0]
    num_commits = len(benchmark_dirs)
    most_recent_dir = max(benchmark_dirs, key=lambda x: Path(x[1]).name)
    timestamp = Path(most_recent_dir[1]).name

    return f"ai-benchmark_{first_commit}_{num_commits}commits_{timestamp}.md"


def main():
    """Main function to run trace-based AI benchmark comparison."""
    base_path = os.path.join(os.path.dirname(__file__), "..", "results", "ai-benchmark")

    print("Scanning AI benchmark directories for trace data...")
    benchmark_dirs = find_benchmark_directories(base_path)

    if not benchmark_dirs:
        print(f"No AI benchmark directories with trace data found in {base_path}")
        return

    print(f"Found {len(benchmark_dirs)} AI benchmark(s) with trace data")
    for commit_hash, dir_path in benchmark_dirs:
        print(f"  - {commit_hash}: {dir_path}")

    print("\nGenerating trace-based AI benchmark comparison table...")
    ...
    print(f"AI benchmark comparison table saved to {output_file}")
    print("\n" + "="*50)
    print(table)
